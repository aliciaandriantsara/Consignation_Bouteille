...
class TransactionModel {
    ...
    public function emprunter(string $qrBouteille, string $cinClient, string $nomBoutique): void {
        $s = $this->db->prepare(
            "INSERT INTO transaction_consignation
             (qr_code_bouteille, CIN_client, nom_boutique, statut)
             VALUES (?, ?, ?, 'EN_COURS')");
    ...
    public function terminer(string $qrBouteille): bool {
        $s = $this->db->prepare(
            "UPDATE transaction_consignation
             SET date_retour = NOW(), statut = 'TERMINEE'
    ...
    public function transactionEnCours(string $qrBouteille): ?array {
        $s = $this->db->prepare(
            "SELECT t.*, c.nom, c.prenom, c.telephone
             FROM transaction_consignation t
             JOIN client c ON c.CIN = t.CIN_client
             WHERE t.qr_code_bouteille = ? AND t.statut = 'EN_COURS'
             LIMIT 1");
        $s->execute([$qrBouteille]);
        return $s->fetch() ?: null;
    }
    ...
    public function byRevendeur(string $nomBoutique): array {
        $s = $this->db->prepare(
            'SELECT t.*, c.nom, c.prenom, c.CIN, b.nom_entreprise
             FROM transaction_consignation t
             JOIN client    c ON c.CIN      = t.CIN_client
             JOIN bouteille b ON b.qr_code  = t.qr_code_bouteille
             WHERE t.nom_boutique = ?
             ORDER BY t.date_emprunt DESC');
